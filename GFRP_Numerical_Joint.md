...
值得一提的是，关于破坏模式，纯胶粘的明确是内聚破坏，所有Frank才采用Cohesive单元CZM模型；
对于纯螺栓和混合连接，因为都涉及到螺栓，所以都是发生剪出破坏，即沿着孔的直径大小切出一块”塞子“一样的形式，剪切面Frank判断主要是由于基体的破坏而产生，模拟中采用单元删除，所以简化为单向板模型可以很好模拟这一点。
...
2.材料参数：
    板的尺寸都是400mm×60mm×10mm，胶层0.5mm厚，采用四分之一模型加速计算速度，网格最小为2mm，孔的直径为16mm，而螺栓的直径为15.8mm，实际实验是完全16对16，但是模拟因为动能太大所有留有间隙。
    GFRP是90%都是0°方向的纤维，所有简化为单向板模型（其实也对应其破坏模式）
...
3.Abaqus的设置细节：
    [纯胶粘连接模型的inp文件](0422_A_14.inp)
    [纯螺栓连接模型的inp文件](0422_Bolt_05.inp)
    [混合连接48mm端距inp文件](0422_H48_02.inp)

4.Abaqus结果与实验结果曲线对比：
[纯胶粘连接对比](Numerical_Curve_1)
[纯螺栓连接对比](Numerical_Curve_2)
[混合连接48mm端距对比](Numerical_Curve_3)
其中，纯胶粘Frank认为主要取决于Cohesive的属性，所以斜率上其实还需要进行一点点的调整；
纯螺栓连接的最不符合，主要还是因为实验曲线的在相对位移0.03-0.05左右之前的曲线大概率是由于摩擦承载刚度较大（这个刚度大概率就是摩擦系数），另外后期就是GFRP的基体在承受剪切面的剪切力，而模拟曲线在相对位移0.14左右之前都是0荷载，意味着之前可能是间隙无接触，但很奇怪Frank设置了预紧力以及摩擦系数0.3的所有面的通用接触，也不至于0荷载（可能Frank提取的是固定端反力这个不一样？）反正极限荷载也差很多。
最后，这个混合连接的刚度差不多，因为涉及到胶层刚度几乎就是胶层决定，而不一样的地方在于极限荷载太低了，所以螺栓预紧力的影响是否影响了胶层的性质？实验与模拟又有什么不同呢？
